...
function cookie(x){
    let cookieEater = ''
    typeof x === 'string' ? cookieEater = 'Zach': typeof x === 'number' ? cookieEater = 'Monica': cookieEater = 'the dog'
...
function procedure(n){
   ...
   return multiples.map(e => e.toString().split('').reduce((a,b)=> Number(a)+Number(b))).reduce((a,b)=> a+b)
}