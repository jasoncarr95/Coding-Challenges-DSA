...
function high(x){
    function score(word){
        return word.split('').map(e => e.charCodeAt()-96).reduce((a,b)=> a + b , 0)
    }
...
function isItANum(str) {
    let nums = str.split('').filter(e => e ==="0" ||  Number(e)).join('')
    return nums.length === 11 && nums.startsWith('0') ? nums : 'Not a phone number'
...
function addArrays(array1, array2) {
    ...
    let arrSum = sum.toString().split('').filter(e => Number(e) || e==='0').map(e => Number(e))
